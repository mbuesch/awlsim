...
static void clkstretch_timer_init(void)
{
	...
	/* The table has one entry for each timer counter state. */
	build_assert(ARRAY_SIZE(clkstretch_release_hint_table) == 256);
	/* ASM code expects table elements to be bytes. */
	build_assert(sizeof(clkstretch_release_hint_table[0]) == 1);
}
...
"1:	ldi r30, lo8(pm(clkstretch_release_hint_table))		\n" \
"	ldi r31, hi8(pm(clkstretch_release_hint_table))		\n" \
"	in __tmp_reg__, %[_TCNT0]				\n" \
"	add r30, __tmp_reg__					\n" \
"	adc r31, __zero_reg__					\n" \
"	lpm r30, Z						\n" \
"	ldi r31, %[_CLKSTRETCH_RELEASE_SAFE]			\n" \
"	cp r30, r31						\n" \
"	brne 1b							\n"
...
#define CLKSTRETCH_TIMER_PREPARE()	""
#define CLKSTRETCH_TIMER_WAIT()		""
#define CLKSTRETCH_RELEASE_SAFE		0
#define CLKSTRETCH_RELEASE_UNSAFE	0
...
#define IN_CONSTR_BASE							\
	...
	[_CLKSTRETCH_RELEASE_SAFE]	"M" (CLKSTRETCH_RELEASE_SAFE),	\
	[_CLKSTRETCH_RELEASE_UNSAFE]	"M" (CLKSTRETCH_RELEASE_UNSAFE)
...
"	rjmp isr_ovf_ret				\n"
	...
	: /* inputs */
		IN_CONSTR_BASE,
		[_sr_send]	"M" (USISR_BASE | (0 << USISIF) | (0 << USICNT0))
	: /* clobbers */
		"memory"
	);
	unreachable();
...
"	rjmp isr_ovf_ret				\n"
	...
	: /* inputs */
		IN_CONSTR_BASE,
		[_sr_rddata]	"M" (USISR_BASE | (0 << USISIF) | (0 << USICNT0))
	: /* clobbers */
		"memory"
	);
	unreachable();
...
//FIXME we should make sure to be earlier
