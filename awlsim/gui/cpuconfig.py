...
class CpuConfigWidget(QWidget):
	def __init__(self, parent=None):
		...
		group = QGroupBox(_("Hardware resource specification"), self)
		group.setLayout(QGridLayout())

		label = QLabel(_("Number of accumulator registers"), self)
		label.setToolTip(
			_("Select the number of ACCU registers the CPU will have.\n"
		...
		self.accuCombo.addItem(_("2 accus"), 2)
		self.accuCombo.addItem(_("4 accus"), 4)
		self.accuCombo.setToolTip(label.toolTip())
		group.layout().addWidget(self.accuCombo, 0, 1)

		label = QLabel(_("Number of S7 timers"), self)
		label.setToolTip(
			_("Set the number of S7 timers the CPU will have.\n"
		...
		label = QLabel(_("Number of S7 counters"), self)
		label.setToolTip(
			_("Set the number of S7 counters the CPU will have.\n"
		...
		label = QLabel(_("Number flag (Merker) memory bytes"), self)
		label.setToolTip(
			_("Set the number flag (Merker) memory bytes the CPU will have."))
		...
		label = QLabel(_("Number process image input bytes"), self)
		label.setToolTip(
			_("Set the number process image input memory bytes the CPU will have."))
		...
		label = QLabel(_("Number process image output bytes"), self)
		label.setToolTip(
			_("Set the number process image output memory bytes the CPU will have."))
		...
		label = QLabel(_("Number local stack memory (TEMP) bytes"), self)
		label.setToolTip(
			_("Set the number local stack memory (TEMP) bytes the\n"
		...
		label = QLabel(_("Parenthesis stack (Klammerstack) size"), self)
		label.setToolTip(
			_("Set the depth of the parenthesis stack (Klammerstack) the\n"
		...
		label = QLabel(_("CALL stack size"), self)
		label.setToolTip(
			_("Set the depth of the CALL stack.\n"
		...
		group = QGroupBox(_("Hardware configuration"), self)
		group.setLayout(QGridLayout())

		label = QLabel(_("Clock memory byte (Taktmerker)"), self)
		label.setToolTip(
			_("Select an M byte for use as clock memory byte (Taktmerker)."))
		group.layout().addWidget(label, 0, 0)
		...
		self.obTempCheckBox = QCheckBox(_("Enable writing of OB &TEMP "
			"entry-variables"), self)
		self.obTempCheckBox.setToolTip(
			_("If this box is not checked the entry variables in the\n"
		...
		label = QLabel(_("Cycle (OB 1) time limit"), self)
		label.setToolTip(
			_("Time limit of one cycle (OB 1) run, in milliseconds.\n"
		...
		label = QLabel(_("Cycle (OB 1) time minimum"), self)
		label.setToolTip(
			_("Cycle time minimum target of one cycle (OB 1) run, in milliseconds.\n"
		...
		group = QGroupBox(_("AWL language"), self)
		group.setLayout(QGridLayout())

		label = QLabel(_("Mnemonics language"), self)
		label.setToolTip(
			_("Select the AWL/STL mnemonics type.\n"
		...
		self.mnemonicsCombo.addItem(_("Automatic"), S7CPUConfig.MNEMONICS_AUTO)
		self.mnemonicsCombo.addItem(_("English"), S7CPUConfig.MNEMONICS_EN)
		self.mnemonicsCombo.addItem(_("German"), S7CPUConfig.MNEMONICS_DE)
		self.mnemonicsCombo.setToolTip(label.toolTip())
		group.layout().addWidget(self.mnemonicsCombo, 0, 1)

		self.extInsnsCheckBox = QCheckBox(_("Enable e&xtended "
			"non-standard instructions"), self)
		self.extInsnsCheckBox.setToolTip(
			_("Enable special Awlsim specific AWL/STL instructions that are not available\n"
		...
		group = QGroupBox(_("Program"), self)
		group.setLayout(QGridLayout())

		self.preDownloadValidationCheckBox = QCheckBox(
			_("Enable pre-download program validation"), self)
		self.preDownloadValidationCheckBox.setToolTip(
			_("Validate program before downloading it to the CPU.\n"
