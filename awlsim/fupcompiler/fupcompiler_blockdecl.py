...
class FupCompiler_BlockDecl(FupCompiler_BaseObj):
	...
	def compile(self, interf):
		...
		def mkIntro(lines, blockTypeDecl):
			lines.append("// ")
			lines.append("// %s generated by Awlsim FUP compiler" % blockTypeDecl)
			lines.append("// version %s" % VERSION_STRING)
			lines.append("// %s (UTC)" % date)
			lines.append("// AWL/STL mnemonics: %s" % (
				     "DE (German)" if mnemonics == S7CPUSpecs.MNEMONICS_DE
